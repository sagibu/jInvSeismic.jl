...
println(omega[end]*maximum(Minv.h)*(1.0./minimum(m)));
...
# if computeTimeDomain
	...
	# DobsTime = Array{Array{Float32,2}}(undef,size(Q,2));
	# for k=1:length(pFor)
		# Dt = fetch(DRF[k]);
...
# DobsOmHat = Array{Array{ComplexF64,2}}(undef,length(omega));
# for k=1:length(omega)
	# DobsOmHat[k] = zeros(ComplexF64,nrcv,nsrc);
# end

# nw = length(rickt);
# rickh = fft(rickt);
# rickhsub = zeros(ComplexF64,length(omega));
